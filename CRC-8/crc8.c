/**
 * @file crc8.c
 * @author Naim MASRI (naimmas@outlook.com)
 * @brief
 * @version 0.1
 * @date 2023-10-21
 *
 * @copyright Copyright (c) 2023
 *
 */
#include "crc8.h"
//crc8 LUT from CRC-8/CDMA2000
static const uint8_t crc8_table[256] = {

};

/**
 * @brief calculate crc8 checksum of given data 
 * @note the crc8 checksum is calculated using the polynomial 0x9B
 * @param data pointer to the data array 
 * @note increasing the size of the data array will increase the probability of collision
 * @param size the size of the data array
 * @return uint8_t 
 */
uint8_t calculate_crc8(volatile const uint8_t *data, uint8_t size)
{
    uint8_t val = 0;
    uint8_t *pos = (uint8_t *)data;
    uint8_t *end = pos + size;

    while (pos < end)
    {
        val = crc8_table[val ^ *pos];
        pos++;
    }

    return val;
}

bool check_crc8(volatile const uint8_t *data, uint8_t size)
{
    return 0==calculate_crc8(data, size);
}